...
// ============================================================
emit_tok:
    adrp x3, ls_token_count@PAGE
    add     x3, x3, ls_token_count@PAGEOFF
    ldr     x4, [x3]
    mov     x5, #12
    madd    x5, x4, x5, xzr
    adrp x6, ls_token_buf@PAGE
    add     x6, x6, ls_token_buf@PAGEOFF
    add     x6, x6, x5
    str     w0, [x6]
    str     w1, [x6, #4]
    str     w2, [x6, #8]
...
// ============================================================
match_kw:
    adrp x3, lex_kw_table@PAGE
    add     x3, x3, lex_kw_table@PAGEOFF
    add     x4, x0, w1, uxtw       // x4 = &src[offset]
.Llex_kw_loop:
    ldrb    w5, [x3]               // kw_len
    cbz     w5, .Llex_kw_miss
    ldrb    w6, [x3, #1]           // kw_tok
...
.Lml_not_2ch:
    ...
    sub     w3, w15, #32
    adrp x5, op1_table@PAGE
    add     x5, x5, op1_table@PAGEOFF
    ldrb    w4, [x5, x3]
...
code_LEX_TOKENS:
    str     x22, [x24, #-8]!
    adrp x22, ls_token_buf@PAGE
    add     x22, x22, ls_token_buf@PAGEOFF
    NEXT
...
code_LEX_TOKEN_FETCH:
    mov     x0, #12
    mul     x0, x22, x0
    adrp x1, ls_token_buf@PAGE
    add     x1, x1, ls_token_buf@PAGEOFF
    add     x1, x1, x0
    ldr     w2, [x1]               // type (u32)
