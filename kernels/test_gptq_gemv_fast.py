...
def main():
    ...
    fast_mod = CUmodule()
    _check("load fast", cuda.cuModuleLoadData(byref(fast_mod), open(os.path.join(KERNEL_DIR, "gptq_gemv_fast.cubin"), "rb").read()))
    ...
    print(f"\nSweeping K_SPLITS (divisors of K_packed={K_packed}):")
    ...
    for K_SPLITS in K_SPLITS_OPTIONS:
        ...
        fast_args = [c_uint64(d_qweight.value), c_uint64(d_scales.value), c_uint64(d_input.value),
                     c_uint64(d_output_fast.value), c_uint32(N), c_uint32(K),
        ...
        _check("launch fast", cuda.cuLaunchKernel(fast_func, fast_grid_x, fast_grid_y, 1, 256, 1, 1,
                                                   fast_smem, None, fast_ptrs, None))
        _check("sync", cuda.cuCtxSynchronize())

        out_fast = bytearray(N * 4)
        _check("DtoH fast", cuda.cuMemcpyDtoH_v2(ctypes.cast((ctypes.c_ubyte * (N*4)).from_buffer(out_fast), c_void_p),
                                                   d_output_fast, c_size_t(N * 4)))
        ...
        # Benchmark (kernel-only, no memset)
        for _ in range(WARMUP):
            cuda.cuLaunchKernel(fast_func, fast_grid_x, fast_grid_y, 1, 256, 1, 1,
                                fast_smem, None, fast_ptrs, None)
        _check("sync", cuda.cuCtxSynchronize())
        ...
        if bw_fast > best_bw and n_wrong == 0:
            best_bw = bw_fast
    ...
    print(f"  Best fast (K_SPLITS={best_splits:3d}):   {best_ms*1000:7.1f} us  {best_bw:7.1f} GB/s")
    print(f"  Speedup: {best_bw/bw_ref:.2f}x")
    print(f"  Data moved per call: {total_bytes/1e6:.2f} MB")
    print(f"  Theoretical min @ 4000 GB/s: {total_bytes / 4e12 * 1e6:.1f} us")
    print(f"  Achieved BW / peak: {best_bw/4000*100:.1f}%")
