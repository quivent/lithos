.global code_CUBIN_PARAMS
.global code_ELF_BUF
.global code_ELF_BUILD_ARM64
.global code_ELF_BUILD_CUBIN
.global code_ELF_POS
.global code_ELF_SAVE
.global code_ELF_WRITE_ARM64
.global code_ELF_WRITE_CUBIN
.global elf_arm64_header
.global elf_arm64_phdr
...
.align 4
eb_emit:
    adrp x1, ls_elf_pos@PAGE
    add     x1, x1, ls_elf_pos@PAGEOFF
    ldr     x2, [x1]               // x2 = pos
    adrp x3, ls_elf_buf@PAGE
    add     x3, x3, ls_elf_buf@PAGEOFF
    strb    w0, [x3, x2]
    add     x2, x2, #1
    str     x2, [x1]
    ret
...
.align 4
put_u16:
    adrp x2, ls_elf_buf@PAGE
    add     x2, x2, ls_elf_buf@PAGEOFF
    strb    w0, [x2, x1]
    lsr     x3, x0, #8
    add     x4, x1, #1
    strb    w3, [x2, x4]
...
.align 4
get_pos:
    adrp x0, ls_elf_pos@PAGE
    add     x0, x0, ls_elf_pos@PAGEOFF
    ldr     x0, [x0]
    ret
...
elf_build_arm64:
    ...
    // Record offset
    bl      get_pos
    adrp x1, arm_text_off@PAGE
    add     x1, x1, arm_text_off@PAGEOFF
    str     x0, [x1]
    ...
    adrp x1, ls_last_comp_addr@PAGE
    add     x1, x1, ls_last_comp_addr@PAGEOFF
    ldr     x1, [x1]
    ...
    adrp x1, arm_text_size@PAGE
    add     x1, x1, arm_text_size@PAGEOFF
    str     x20, [x1]
    ...
    // Record offset
    bl      get_pos
    adrp x1, arm_data_off@PAGE
    add     x1, x1, arm_data_off@PAGEOFF
    str     x0, [x1]
    // Copy data
    mov     x0, x21
    mov     x1, x22
    bl      emit_bytes
    // Record size
    adrp x1, arm_data_size@PAGE
    add     x1, x1, arm_data_size@PAGEOFF
    str     x22, [x1]
.Larm_no_data:

    // .bss has no file content — accounted for in p_memsz only
    adrp x1, arm_bss_size@PAGE
    add     x1, x1, arm_bss_size@PAGEOFF
    str     x23, [x1]
...
elf_save:
    ...
    adrp x20, ls_elf_buf@PAGE
    add     x20, x20, ls_elf_buf@PAGEOFF
    adrp x1, ls_elf_pos@PAGE
    add     x1, x1, ls_elf_pos@PAGEOFF
...
elf_build_cubin:
    ...
    adrp x0, sn_shstrtab_str@PAGE
    add     x0, x0, sn_shstrtab_str@PAGEOFF
    bl      emit_asciz
    ...
    adrp x0, sn_strtab_str@PAGE
    add     x0, x0, sn_strtab_str@PAGEOFF
    bl      emit_asciz
    ...
    adrp x0, sn_symtab_str@PAGE
    add     x0, x0, sn_symtab_str@PAGEOFF
    bl      emit_asciz
    ...
    adrp x0, sn_nvinfo_str@PAGE
    add     x0, x0, sn_nvinfo_str@PAGEOFF
    bl      emit_asciz
    ...
    adrp x0, sn_nvinfo_k_pfx@PAGE
    add     x0, x0, sn_nvinfo_k_pfx@PAGEOFF
    mov     x1, x20
    mov     x2, x21
    ...
    adrp x0, sn_text_k_pfx@PAGE
    add     x0, x0, sn_text_k_pfx@PAGEOFF
    mov     x1, x20
    mov     x2, x21
    bl      emit_prefix_kernel
    ...
    adrp x0, sn_const0_k_pfx@PAGE
    add     x0, x0, sn_const0_k_pfx@PAGEOFF
    mov     x1, x20
    mov     x2, x21
    bl      emit_prefix_kernel
    ...
    adrp x0, sn_shared_str@PAGE
    add     x0, x0, sn_shared_str@PAGEOFF
    bl      emit_asciz
...
2:
    ...
    // [0] NULL section
    adrp x0, shdr_scratch@PAGE
    add     x0, x0, shdr_scratch@PAGEOFF
    stp     xzr, xzr, [x0, #0]
    stp     xzr, xzr, [x0, #16]
    stp     xzr, xzr, [x0, #32]
    stp     xzr, xzr, [x0, #48]
...
elf_write_cubin_simple:
    ...
    // Fill cubin_params
    adrp x8, cubin_params@PAGE
    add     x8, x8, cubin_params@PAGEOFF
...
code_ELF_BUF:
    str     x22, [x24, #-8]!
    adrp x22, ls_elf_buf@PAGE
    add     x22, x22, ls_elf_buf@PAGEOFF
    NEXT
...
code_ELF_POS:
    str     x22, [x24, #-8]!
    adrp x22, ls_elf_pos@PAGE
    add     x22, x22, ls_elf_pos@PAGEOFF
    NEXT
...
code_CUBIN_PARAMS:
    str     x22, [x24, #-8]!
    adrp x22, cubin_params@PAGE
    add     x22, x22, cubin_params@PAGEOFF
    NEXT
