#!/usr/bin/env python3
"""Test add_store kernel: output[i] = a[i] + b[i]."""
...
def test(N, label):
    mod = cuda.module_from_file(os.path.join(KERNEL_DIR, "add_store.cubin"))
    func = mod.get_function("add_store")
    ...
    print(f"add_store {label} (N={N}): max_err={max_err:.6e}", end=" ")
    if max_err < 1e-6:
        print("PASS")
    else:
        print("FAIL")
