...
var LITHOS_GLOSSARY = {
  ...
  "cubin": "A compiled GPU binary in ELF format containing SASS machine code. Lithos generates cubins by emitting SASS directly into the ELF structure — no ptxas in the pipeline. A Lithos cubin is ~4 KB; the cuBLAS atlas is ~800 MB.",
  ...
  "Forth": "A stack-based programming language invented by Charles Moore. Lithos is Forth-derived: programs are sequences of words that manipulate a data stack. Each Lithos word emits SASS instructions. Compilation is direct binary emission, taking microseconds.",
  ...
  "Kernel": "A function that runs on the GPU, executed by thousands of threads in parallel. In Lithos, kernels are compiled from Forth words directly into SASS. A fused kernel combines multiple operations (e.g., norm + projection + activation) into a single launch.",
  ...
  "Lithos": "A Forth-derived GPU compute language that emits SASS directly into cubins for NVIDIA GPUs. Built for inference on GH200. Replaces PyTorch, cuBLAS, cuDNN, and the CUDA runtime with direct kernel compilation in microseconds. Named from lithography — writing on silicon.",
  ...
  "PTX": "Parallel Thread Execution. NVIDIA's virtual assembly language. Stable text ISA. Gets compiled to SASS by the driver (ptxas) at load time. Lithos bypasses PTX entirely, emitting SASS directly into cubin files.",
  "ptxas": "NVIDIA's PTX assembler. Compiles PTX to SASS. Runs inside the driver when you call cuModuleLoadData with PTX. Typical compile time: 100ms-2s per kernel. Lithos eliminates this step by emitting SASS directly into cubin ELF files.",
  ...
  "SASS": "Shader ASSembly. The actual GPU machine code that runs on NVIDIA hardware. Each instruction is 16 bytes: an 8-byte instruction word (opcode, operands) and an 8-byte control word (stall counts, barriers, register reuse). Lithos emits SASS directly into cubins, bypassing ptxas.",
  ...
  "SiLU": "Sigmoid Linear Unit. The activation function x * sigmoid(x), computed as x / (1 + exp(-x)). Used in every MLP layer of Qwen 3.5. Lithos computes it in 6 SASS instructions entirely in registers using ex2.approx, with no intermediate memory access.",
  ...
  "Hardware-Matched Quantization": "A quantization design approach where the bit layout and scale granularity are chosen to match a specific GPU's on-chip memory capacity and compute unit mix, rather than being determined solely by model quality or storage efficiency. The target: fit one model layer exactly in on-chip memory (~66 MB on Hopper) at ~2.93 bits/weight, and balance dequantization work across INT32, FP32, tensor core, and FP64 units in a pipelined schedule."
};
