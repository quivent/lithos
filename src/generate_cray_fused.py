#!/usr/bin/env python3
"""
Cray-optimized multi-block cooperative fused forward pass benchmark.

132 blocks x 256 threads, ALL 132 SMs active simultaneously.
Uses shared memory input caching + vectorized weight loads.

Run:
    python3 /home/ubuntu/lithos/src/generate_cray_fused.py
"""
...
# ---------------------------------------------------------------------------
# Architecture constants (must match forward_pass_multi.ptx)
# ---------------------------------------------------------------------------
...
def main() -> int:
    banner("CRAY-OPTIMIZED MULTI-BLOCK COOPERATIVE FUSED FORWARD PASS")
    ...
    # ------------------------------------------------------------------
    # 2. Load forward_pass_multi.cubin
    # ------------------------------------------------------------------
    banner("Loading forward_pass_cray.cubin")
    t0 = time.monotonic()
    cubin_path = f"{KERNEL_DIR}/forward_pass_cray.cubin"
    mod = gpu.load_cubin(cubin_path)
    func = gpu.get_function(mod, "forward_pass_cray")
    ...
    # ------------------------------------------------------------------
    # 5. Allocate multi-block specific buffers
    # ------------------------------------------------------------------
    ...
    # LM head (same as single-block: FP16 data passed as GPTQ placeholder)
    ...
    # ------------------------------------------------------------------
    banner("LAUNCHING MULTI-BLOCK COOPERATIVE KERNEL")
    ...
    print("  Warmup launch...")
    try:
        # Reset sync workspace before each launch
        gpu.memset_d8(d_sync_workspace, 0, 16)
    ...
    print()
    print(f"  --- Multi-Block Configuration ---")
    ...
    print()
    print(f"  --- Comparison ---")
    print(f"  Single-block fused:  ~4081 ms  (1 SM / 132)")
    print(f"  Multi-block orig:    ~135 ms   (no smem caching)")
    print(f"  Cray-optimized:      {median_ms:.3f} ms ({grid_size} SMs / {sm_count})")
    print(f"  Python pipeline:     ~2500 ms  (97% overhead)")
    if median_ms > 0:
        speedup_vs_python = 2500.0 / median_ms
        print(f"  Speedup vs Python:   {speedup_vs_python:.0f}x")
    print()
    print(f"  Logits valid:        {'YES' if logits_valid else 'NO (all-zero or NaN)'}")
