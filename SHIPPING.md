...
| Parse + emit SASS + wrap + `cuModuleLoadData` for both kernels | ~10–100 ms |
...
A kernel is bytes the GPU executes. SASS instructions, raw 128 bits each on Hopper. That's what the hardware runs.

The NVIDIA driver won't accept loose SASS — it wants a tiny metadata wrapper (register count, parameter layout, entry point symbol) in an ELF format called a "cubin." The word "cubin" is just "kernel in the format the driver wants."
...
- A compiler that turns the language into SASS
