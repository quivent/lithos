...
def main() -> int:
    banner("MULTI-BLOCK COOPERATIVE FUSED FORWARD PASS")
    ...
    # ------------------------------------------------------------------
    banner("Loading forward_pass_multi.cubin")
    t0 = time.monotonic()
    cubin_path = f"{KERNEL_DIR}/forward_pass_multi.cubin"
    mod = gpu.load_cubin(cubin_path)
    func = gpu.get_function(mod, "forward_pass_multi")
    ...
    print(f"  Multi-block fused:   {median_ms:.3f} ms ({grid_size} SMs / {sm_count})")
